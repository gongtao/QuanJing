...
static const float M_PI_X_2 = PI * 2.0f;

template <typename PeriodNumType>
inline
float ElasticEaseIn (const float& t)
{
    if (t == 0 || t == 1)
    {
        return t;
    ...
    {
        const static float s = PeriodNumType::Value * 0.25f;
        float tt = t - 1;
        return - std::pow(2, 10 * tt) * std::sin( (tt - s) * M_PI_X_2 / PeriodNumType::Value);
    }
}

template <typename PeriodNumType>
inline
float ElasticEaseOut(const float& t)
{
    if (t == 0.f || t == 1.f)
    {
        return t;
    ...
    {
        const static float s = PeriodNumType::Value * 0.25f;
        return std::pow(2.f, -10.f * t) * std::sin((t - s) * M_PI_X_2 / PeriodNumType::Value) + 1.f;
    }
}

template <typename PeriodNumType>
inline
float ElasticEaseInOut(const float& t)
{
    //AssertSTATIC (PeriodNumType::Value != 0.0f);
    ...
    {
        float tt = t * 2;
        static const float s = PeriodNumType::Value * 0.25;
        
        tt = tt - 1;
        if ( tt < 0 )
        {
            return -0.5f * pow(2, 10 * tt) * sin((tt - s) * M_PI_X_2 / PeriodNumType::Value);
        }
        else
        {
            return pow(2, -10 * tt) * sin((tt - s) * M_PI_X_2 / PeriodNumType::Value) * 0.5f + 1;
        }
    }
}

struct ElasticDefaultPeriodType  { static const float Value; };
const float ElasticDefaultPeriodType::Value = 0.5;
...
{
    return ElasticEaseIn<ElasticDefaultPeriodType>(p);
}

// Modeled after the damped sine wave y = sin(-13pi/2*(x + 1))*pow(2, -10x) + 1
AHFloat ElasticEaseOut(AHFloat p)
{
    return ElasticEaseOut<ElasticDefaultPeriodType>(p);
}
...
{
    return ElasticEaseInOut<ElasticDefaultPeriodType>(p);
}

// Modeled after the overshooting cubic y = x^3-x*sin(x*pi)
AHFloat BackEaseIn(AHFloat p)
{
    static const float overshoot = 1.70158f;
...
AHFloat BackEaseOut(AHFloat p)
{
    ...
    static const float overshoot = 1.70158f;
    float pp = p - 1;
    return pp * pp * ((overshoot + 1) * pp + overshoot) + 1;
#endif
}
...
{
    static const float overshoot = 1.70158f * 1.525f;
    
    float pp = p * 2;
    if (pp < 1)
    {
        return (pp * pp * ((overshoot + 1) * pp - overshoot)) * 0.5;
    }
    else
    {
        pp = pp - 2;
        return (pp * pp * ((overshoot + 1) * pp + overshoot)) * 0.5 + 1;
    }
}
