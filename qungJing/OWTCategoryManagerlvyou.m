...
@interface OWTCategoryManagerlvyou()

@property (nonatomic, strong) NSDate* lastRefreshTime;
@property (nonatomic, strong) NSMutableArray *categoriesTmp;

@end

@implementation OWTCategoryManagerlvyou
...
- (void)setup
{
    ...
    NSString *homePath  = [homeDictionary stringByAppendingString:@"/Documents/cachelvyou.archiver"];
...
- (void)refreshCategories2WithSuccess:(void(^)())success
                              failure:(void(^)(NSError* error))failure
{
    _lastRefreshTime = [NSDate date];
    NSLog(@"_keyPath =%@",_keyPath);
    if (_keyPath ==0) {
        RKObjectManager* om = [RKObjectManager sharedManager];//feeds/gwly
        [om getObject:nil
                 path:@"travel-domestic_new"
           parameters:nil
              success:^(RKObjectRequestOperation* o, RKMappingResult* result) {
                  ...
                  [self updateWithCategoryDatas:categorieDatas finish:YES];
...
- (void)refreshCategoriesWithSuccess:(void(^)())success
                             failure:(void(^)(NSError* error))failure
{
    _categoriesTmp = [[NSMutableArray alloc]init];
    _lastRefreshTime = [NSDate date];
    NSLog(@"_keyPath =%@",_keyPath);
    if (_keyPath ==0) {
        RKObjectManager* om = [RKObjectManager sharedManager];//feeds/gwly
        [om getObject:nil
                 path:@"cdn2/travel-international_new"
           parameters:nil
              success:^(RKObjectRequestOperation* o, RKMappingResult* result) {
                  [o logResponse];
                  ...
                  [self updateWithCategoryDatas:categorieDatas finish:NO];
                  //继续去请求国内旅游数据
                  if (success != nil) {
                      [self refreshCategories2WithSuccess:success
                                                  failure:failure];
                  }
              }
              failure:^(RKObjectRequestOperation* o, NSError* error) {
...
- (void)updateWithCategoryDatas:(NSArray*)categorieDatas finish:(BOOL)isFinish
{
