...
NSString* const kWaterFlowElementKindSectionHeader = @"kWaterFlowElementKindSectionHeader";
NSString* const kWaterFlowElementKindSectionFooter = @"kWaterFlowElementKindSectionFooter";

@interface OWaterFlowLayout ()
...
@implementation OWaterFlowLayout
...
- (void)prepareLayout
{
    ...
    self.layoutDataSource = (id<OWaterFlowLayoutDataSource>)self.collectionView.dataSource;
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section)
    {
        ...
        if ([self.layoutDataSource respondsToSelector:@selector(collectionView:layout:heightForHeaderInSection:)])
        {
            headerHeight = [self.layoutDataSource collectionView:self.collectionView layout:self heightForHeaderInSection:section];
        }
        else
        {
        ...
        if (headerHeight > 0)
        {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:kWaterFlowElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
        ...
        for (idx = 0; idx < itemCount; idx++)
        {
            ...
            CGSize itemSize = [self.layoutDataSource collectionView:self.collectionView layout:self sizeForItemAtIndexPath:indexPath];
        ...
        if ([self.layoutDataSource respondsToSelector:@selector(collectionView:layout:heightForFooterInSection:)])
        {
            footerHeight = [self.layoutDataSource collectionView:self.collectionView layout:self heightForFooterInSection:section];
        }
        else
        {
        ...
        if (footerHeight > 0)
        {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:kWaterFlowElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (UICollectionViewLayoutAttributes*)layoutAttributesForSupplementaryViewOfKind:(NSString*)kind atIndexPath:(NSIndexPath*)indexPath
{
    ...
    if ([kind isEqualToString:kWaterFlowElementKindSectionHeader])
    {
        attribute = self.headersAttribute[@(indexPath.section)];
    }
    else if ([kind isEqualToString:kWaterFlowElementKindSectionFooter])
    {
