...
-(void)likeBtnClick:(UIButton *)sender
{
    ...
    if (sender.selected==NO) {
        ...
        [um postObject:nil path:@"activity/like" parameters:@{
                                                              @"Activityid":activityid} success:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
    ...
    }else
    {
        ...
        [_viewContoller reloadData:_number];
        [um postObject:nil path:@"activity/like" parameters:@{
                                                              @"Activityid":activityid} success:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {} failure:^(RKObjectRequestOperation *operation, NSError *error) {}];
    }
}
...
-(void)onReplyTap:(UITapGestureRecognizer *)sender
{
    ...
    if ([ljcomment.userid isEqualToString:GetUserManager().currentUser.userID ]) {
        ...
        _viewContoller.replyid=ljcomment.userid;}
    commentcb(_activity,_number);
}
-(void)onTap3
{
