//
//  OWTUserDownloadedAssetsViewCon.m
//  Weitu
//
...
#import "OWTUserDownloadedAssetsViewCon.h"
...
@interface OWTUserDownloadedAssetsViewCon ()
{
}
...
@implementation OWTUserDownloadedAssetsViewCon
...
{
    if (self.user != nil && self.user.assetsInfo != nil && self.user.assetsInfo.downloadedAssets != nil)
    {
        return self.user.assetsInfo.downloadedAssets;
    }
    else
    {
        return nil;
...
- (void)setup
{
    ...
    __weak OWTUserDownloadedAssetsViewCon* wself = self;
    ...
    _assetViewCon.refreshDataFunc = ^(void (^refreshDoneFunc)())
    {
        ...
        [um refreshUserDownloadedAssets:wself.user
                            success:^{
    ...
    {
        OWTUserManager* um = GetUserManager();
        [um loadMoreUserDownloadedAssets:wself.user
                               count:50
                             success:^{
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self refreshUserDownloadedAssetsIfNeeded];
}
...
- (void)refreshUserDownloadedAssetsIfNeeded
{
    if (_user == nil)
    {
        return;
    ...
    NSMutableOrderedSet* downloadedAssets = [self assets];
...
- (void)setUser:(OWTUser*)user
{
    ...
    self.navigationItem.title = [NSString stringWithFormat:@"%@分享的照片", _user.displayName];
}
