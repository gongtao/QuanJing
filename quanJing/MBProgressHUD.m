...
- (void)drawRect:(CGRect)rect {
	...
	// Center HUD  by pandara
	// Draw rounded HUD backgroud rect
    if (!_shouldMakeBackgroundClear) {
...
//		_backgroundTintColor = [[UIColor alloc] initWithWhite:1.f alpha:.1f];
        _backgroundTintColor = [[UIColor alloc] initWithRed:0 green:0 blue:0 alpha:0.7f];//圈圈的背景 by pandara
		[self registerForKVO];
	}
	return self;
}
...
- (void)drawRect:(CGRect)rect {
	...
	if (_annular) {
		...
		processPath.lineWidth = lineWidth - 2;//pandara
