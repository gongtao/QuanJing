...
- (instancetype)initWithGroupId:(NSString *)chatGroupId
{
    ...
    _groupOccupantsCount = chatGroup.groupOccupantsCount;
...
- (void)viewDidLoad
{
    ...
    [self fetchGroupInfo];
    //self.title = [self.title stringByAppendingString:[NSString stringWithFormat:@"(%i人)",_groupOccupantsCount]];
}
...
- (UIScrollView *)scrollView
{
    if (_scrollView == nil) {
        ...
        _addButton = [[UIButton alloc] init ];//WithFrame:CGRectMake(0, 0, kContactSize - 10, kContactSize - 10)];
        ...
        _removeButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, kContactSize - 10, kContactSize - 10)];
        [_removeButton setImage:[UIImage imageNamed:@"faceDelete"] forState:UIControlStateNormal];
        [_removeButton setImage:[UIImage imageNamed:@"faceDelete"] forState:UIControlStateHighlighted];
        [_removeButton addTarget:self action:@selector(removeContact:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIView *)footerView
{
    if (_footerView == nil) {
        ...
        self.dissolveButton.frame = self.clearButton.frame;//CGRectMake(20, CGRectGetMaxY(self.clearButton.frame) + 30, _footerView.frame.size.width - 40, 35);
        
        self.exitButton.frame = self.clearButton.frame;//CGRectMake(20, CGRectGetMaxY(self.clearButton.frame) + 30, _footerView.frame.size.width - 40, 35);
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.row == 3)
    {
        ...
        BOOL isOwner = [_chatGroup.owner isEqualToString:loginUsername];
        if (!isOwner) {
            TTAlertNoTitle(@"群主你在哪? 快回来改昵称");
            return;
        }
        
        GroupSubjectChangingViewController *changingController = [[GroupSubjectChangingViewController alloc] initWithGroup:_chatGroup];
        changingController.updateGropChatTitle = ^(NSString *title){
            self.title = title;
        };
        ...
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                if(showAddButton && index == self.dataSource.count)
                {
                    ...
                    self.removeButton.frame = CGRectMake(j * kContactSize + 5+kContactSize, i * kContactSize + 10+kContactSize, kContactSize - 10, kContactSize - 10);
                }
...
{
    AddFriendListViewController *addListVC = [[AddFriendListViewController alloc] init];
     addListVC.existMenberArray = _chatGroup.occupants;
    addListVC.addFriendToGrop = ^(NSArray *newMenber){
        [self newMenber:newMenber];
    };
...
-(void)newMenber:(NSArray*)menber
{
    ...
    NSDictionary *loginInfo = [[[EaseMob sharedInstance] chatManager] loginInfo];
    NSString *username = [loginInfo objectForKey:kSDKUsername];
    NSString *messageStr = [NSString stringWithFormat:NSLocalizedString(@"group.somebodyInvite", @"%@ invite you to join group \'%@\'"), username, self.chatGroup.groupSubject];
    EMError *error = nil;
    _chatGroup = [[EaseMob sharedInstance].chatManager addOccupants:menber toGroup:self.chatGroup.groupId welcomeMessage:messageStr error:&error];
    if(_chatGroup)
    {   [self fetchGroupInfo];
        [self.navigationController popViewControllerAnimated:YES];
    }
    else{
        [weakSelf showHint:@"添加失败"];
        [self.navigationController popViewControllerAnimated:YES];
