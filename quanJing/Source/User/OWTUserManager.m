	...
	failure:(void (^)(NSError *))failure
{
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		[[QJPassport sharedPassport] requestUserInfo:^(QJUser * _Nonnull user, NSDictionary * _Nonnull userDic, NSError * _Nonnull error) {
	...
	failure:(void (^)(NSError *))failure
{
	if (!user.isPublicInfoAvailable)
		...
		parameters:@{@"startIndex" : [NSString stringWithFormat:@"%li", startIndex],
