...
- (NSDictionary *)uploadByImageData:(NSData *)imageData extension:(NSString *)extension
{
	__block NSDictionary * imageInfo = nil;
	
	[[QJInterfaceManager sharedManager] requestImageTempData:imageData
	extension:extension
	finished:^(NSDictionary * imageDic, NSError * error) {
		if (!error && imageDic && [imageDic isKindOfClass:[NSDictionary class]])
			imageInfo = imageDic;
	}];
	return imageInfo;
}
...
- (void)uploadImages:(id)sender
{
	...
	[SVProgressHUD showWithStatus:@"上传第1张图片" maskType:SVProgressHUDMaskTypeGradient];
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		...
		if (imageInfoArray.count == imageInfos.count)
			[[QJInterfaceManager sharedManager] requestPostAction:imageInfoArray
			albumId:_belongingAlbums.anyObject
			title:_addDesCell.textView.text
			tag:_caption
